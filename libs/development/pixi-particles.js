...
(function(exports, pixi) {
      ...
      if (ease) {
        this.ease =
          typeof ease === 'function'
            ? ease
            : exports.ParticleUtils.generateEase(ease);
      } else {
        this.ease = null;
      }
    }
    ...
    PropertyNode.createList = function(data) {
      if ('list' in data) {
        ...
        var _a = array[0],
          ...
          time = _a.time;
        // eslint-disable-next-line max-len
        var first = (node = new PropertyNode(
          typeof value === 'string'
            ? exports.ParticleUtils.hexToRGB(value)
            : value,
          time,
          data.ease
        ));
          ...
          for (var i = 1; i < array.length; ++i) {
            ...
            node.next = new PropertyNode(
              typeof value_1 === 'string'
                ? exports.ParticleUtils.hexToRGB(value_1)
                : value_1,
              time_1
            );
            node = node.next;
      ...
      var start = new PropertyNode(
        typeof data.start === 'string'
          ? exports.ParticleUtils.hexToRGB(data.start)
          : data.start,
        0
      );
      ...
      if (data.end !== data.start) {
        start.next = new PropertyNode(
          typeof data.end === 'string'
            ? exports.ParticleUtils.hexToRGB(data.end)
            : data.end,
          1
        );
      }
  ...
  (function(ParticleUtils) {
    ...
    /**
     * Combines separate color componets (0-255) into a single uint color.
    ...
    function createSteppedGradient(list, numSteps) {
      ...
      var first = new PropertyNode(
        ParticleUtils.hexToRGB(list[0].value),
        list[0].time
      );
  ...
  function intColorStepped(lerp) {
    ...
    var curVal = this.current.value;
    return exports.ParticleUtils.combineRGBComponents(
      curVal.r,
      curVal.g,
      curVal.b
    );
  ...
  var Particle = /** @class */ (function(_super) {
    ...
    Particle.prototype.init = function() {
      ...
      this.tint = exports.ParticleUtils.combineRGBComponents(
        color.r,
        color.g,
        color.b
      );
  ...
  var Emitter = /** @class */ (function() {
    ...
    Emitter.prototype.init = function(art, config) {
      ...
      if (config.speed) {
        ...
        this.minimumSpeedMultiplier =
          ('minimumSpeedMultiplier' in config
            ? config.minimumSpeedMultiplier
      ...
      if (config.scale) {
        ...
        this.minimumScaleMultiplier =
          ('minimumScaleMultiplier' in config
            ? config.minimumScaleMultiplier
            : config.scale.minimumScaleMultiplier) || 1;
      } else {
      ...
      // get the blend mode
      this.particleBlendMode = exports.ParticleUtils.getBlendMode(
        config.blendMode
      );
      ...
      if (config.ease) {
        this.customEase =
          typeof config.ease === 'function'
            ? config.ease
            : exports.ParticleUtils.generateEase(config.ease);
      } else {
        this.customEase = null;
      }
    ...
    Emitter.prototype.parseSpawnType = function(config) {
      ...
      switch (config.spawnType) {
        case 'rect':
          ...
          this.spawnRect = new pixi.Rectangle(
            spawnRect.x,
            spawnRect.y,
            spawnRect.w,
            spawnRect.h
        ...
        case 'circle':
          ...
          this.spawnCircle = new pixi.Circle(
            spawnCircle.x,
            spawnCircle.y,
            spawnCircle.r
          );
    ...
    Emitter.prototype._spawnRing = function(p, emitPosX, emitPosY) {
      ...
      if (spawnCircle.minRadius !== spawnCircle.radius) {
        helperPoint.x =
          Math.random() * (spawnCircle.radius - spawnCircle.minRadius) +
